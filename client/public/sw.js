...
self.addEventListener('sync', (event) => {
  if (event.tag === 'post-vibe') {
    event.waitUntil(syncVibePost());
  }
});

async function syncVibePost() {
  try {
    // Get pending posts from indexedDB or localStorage
    // and attempt to sync them
    console.log('Syncing pending vibe posts...');
  } catch (error) {
...
self.addEventListener('push', (event) => {
  const options = {
    body: event.data ? event.data.text() : 'New vibe notification!',
